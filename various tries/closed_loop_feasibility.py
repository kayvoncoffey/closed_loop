...
class system_model:
    ...
    def predicted_g(self,tau):
        ...
        low = G_N - tau[self.N:self.N*2]
        high = tau[2*self.N:self.N*3] - G_N
        
        return np.concatenate((G_N,low,high),axis=0)
...
def solve_mpc(tau_ini,N,system_model,g,G_t_ref):
    ...
    G_t_constraint = NonlinearConstraint(system_model.predicted_g, 0.0, np.inf)
    ...
    bounds = [(0, 1000) for idx in range(N)]+[(0,200*g) for idx in range(2*N)]
...
def mpc_cost(tau, system_model,g, N, G_t_ref=100):
    ...
    return np.sum(np.square((np.array(G_t_N)-G_t_ref*g)/(G_t_ref*g)))
...
low = np.zeros(N_iterations)
high = np.zeros(N_iterations)
# ---------- CONTROL SYSTEM CALIBRATION ----------
# Model predictive control horizon
N = 12
tau_ini = np.zeros(3*N)
